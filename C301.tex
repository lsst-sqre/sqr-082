...
To enable anyone to be a successful documentation, we invested in creating a documentation platform that is simple and convenient to author and publish in.
Although there are many valid frameworks for creating and publishing documentation, at Rubin we were attracted to the \emph{docs-like-code} \citep{Gentle2022} framework.
...
For example, documentation for the LSST Science Pipelines can only be be built in the bespoke Jenkins environment.

LSST the Docs is designed to be serverless from the end-user's perspective.
...
The template system is built upon Cookiecutter, augmented with our own Templatekit package (\url{https://github.com/lsst-sqre/templatekit}).
...
By integrating templates with a Slack chatbot, templates are accessible to anyone in the Rubin organization.
...
Our documentation crawling service, Ook (\url{https://github.com/lsst-sqre/ook}), receives push events from updates on the documentation platform.
...
As of writing, the Algolia full-text search only includes documents hosted on \texttt{lsst.io}, but we intend to begin including multi-page guides and event documentation content from other platforms in the near future.
...
Documentation tools like Sphinx can be overwhelming for a casual documentation contributor to set up.
...
To alleviate this concern from individual projects, and also promote standardization across all our documentation projects, Rubin provides the Documenteer Python package (\url{https://documenteer.lsst.io}) for our Sphinx configuration, extension dependencies, custom extensions, and theming.
With a configuration module base, the conf.py files for individual projects is reduced to a single single (this approach is inspired by sphinx-astropy, \citet{thomas_robitaille_2023_8015243}).
...
We are now providing a Python package called \texttt{Technote} (https://github.com/lsst-sqre/technote) that provides Sphinx theme extensions, and configurations that enable Sphinx to generate these single-page articles.
% Rubin's Documenteer builds upon this generic \texttt{Technote} package for our specific branding requirements.
LaTeX is also popular within Rubin Observatory.
To accomodate LaTeX documents we created a package called \texttt{Lander} (https://github.com/lsst-sqre/lander) that generates an HTML wrapper around any PDF in an iframe.
\texttt{Lander} introspects the LaTeX source as well as the repository context to build a metadata set that not only populates the HTML landing page, but also yields machine-readable metadata.
The machine-readable metadata exported by both \texttt{Technote} and \texttt{Lander} allows documents to self-curate when indexed into the documentation hub by \texttt{Ook} (see \ref{sec:hub}).
...
We are now working towards refining these components, particularly LSST the Docs, Technote, and Lander, as generic software that can be built upon with plugins or used as software-as-a-service by other organizations pursuing a docs-like-code journey.
