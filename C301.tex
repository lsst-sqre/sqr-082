...
We are now providing a Python package called \texttt{technote} that provides Sphinx theme extensions, and configurations that enable Sphinx to generate these single-page articles.
Rubin's Documenteer builds upon this generic technote package for our specific branding requirements.
LaTeX is also popular within Rubin Observatory.
To accommodate PDF documents, we created a package called \texttt{Lander} that generates an HTML wrapper around any PDF iframe.
...
This has demonstrated how a refined developer experience helps the team do the right thing and write durable documentation, rather than leave important information to Slack chats, email, and other personal silos.
